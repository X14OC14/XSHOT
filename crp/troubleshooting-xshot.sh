...
screenshots_path="/sdcard/Pictures/Screenshot"  # ini masih pake path gw, lu sesuaikan lagi sama punya lu sendiri
screenshots_backup="${screenshots_path}/backup"
screenshots_path_alt="/sdcard/Pictures/Game Space Screenshot"  # game screenshot path buat CRP
...
autoshot() {
  ...
  echo -e "   ${r}[${y}${count}${r}]"
  echo -e "   $(log)${b}Waiting new file"
  inotifywait -m -e moved_to "$path" "$screenshots_path_alt" 2> /dev/null | \
...
main() {
  dir_name=$(echo -e "${filename}" | awk '{print $1}')
  file_name=$(echo -e "${filename}" | awk '{print $3}')

  # skip android temporary files
  [[ "$file_name" == .pending* ]] && return

  # skip folder events (inotifywait kadang nge-detect subfolder bukan file)
  [[ "$file_name" == */ ]] && return
